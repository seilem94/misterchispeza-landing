...
export default function ContactForm() {
  ...
  // Cargar script Turnstile UNA SOLA VEZ
  useEffect(() => {
    if (!siteKey) return
  ...
  useEffect(() => {
    if (!scriptLoaded || !siteKey || !turnstileRef.current) return

    // Esperar a que window.turnstile esté disponible
    const checkTurnstile = setInterval(() => {
      // ✅ Ahora TypeScript conoce el tipo window.turnstile
      if (typeof window !== 'undefined' && window.turnstile) {
        clearInterval(checkTurnstile)
        
        // Renderizar el widget si el contenedor está vacío
        if (turnstileRef.current && !turnstileRef.current.hasChildNodes()) {
          try {
            // ✅ TypeScript autocompleta y valida las opciones
    ...
    // Cleanup del interval después de 5 segundos
    const timeout = setTimeout(() => clearInterval(checkTurnstile), 5000)
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="mt-4 w-full rounded-2xl bg-amber-500 px-6 py-3 font-semibold text-slate-900 hover:bg-amber-400 disabled:opacity-60 transition-colors"
      >
